...
public static class PhonemeScoringEngine
{
    ...
    private static float LENGTH_WEIGHT = 0.15f;  // phoneme length penalty
    ...
    // Method to set weights dynamically
    public static void SetWeights(float phonemeWeight, float vowelWeight, float lengthWeight)
    {
        MAIN_WEIGHT = phonemeWeight;
        VOWEL_WEIGHT = vowelWeight;
        LENGTH_WEIGHT = lengthWeight;
        
        Debug.Log($"Scoring weights updated - Phoneme: {MAIN_WEIGHT}, Vowel: {VOWEL_WEIGHT}, Length: {LENGTH_WEIGHT}");
    }
